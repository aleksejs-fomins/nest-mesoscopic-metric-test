...
pwd_core = os.path.dirname(os.path.dirname(pwd_lib))
pwd_rez = os.path.join(os.path.join(pwd_core, "data/"), "sim-ds-mat")

# Set paths
sys.path.append(pwd_lib)

# Load user libraries
from scipy_lib import loadmat
from spike2ca import approxDelayConv
...
#print("Saving data .mat file")
#path_datafile = os.path.join(pwd_rez, "source_selftest_rand.mat")
#scipy.io.savemat(path_datafile, {"data" : src_data})
...
#subprocess.run(["matlab", "-nodisplay", "-nosplash", "-nodesktop", "-r", "run('nifty_wrapper.m');exit;"])
...
#rez_data = loadmat(os.path.join(pwd_rez, "results_selftest_rand.mat"))
...
#dim = rez_data['results']['TE_table'].shape
...
##for i in range(dim[0]):
    ##for j in range(dim[1]):
        ##if i != j:
            ##ax[0].plot(rez_data['results']['TE_table'][i][j])
            ##ax[1].plot(rez_data['results']['p_table'][i][j])
...
#for i in range(dim[0]):
    #for j in range(dim[1]):
        #if i != j:
            #te_1D += list(rez_data['results']['TE_table'][i][j])
            #p_1D += list(rez_data['results']['p_table'][i][j])
